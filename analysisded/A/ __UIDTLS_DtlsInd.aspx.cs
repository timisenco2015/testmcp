```json
{
  "filename": "__UIDTLS_DtlsInd.aspx.cs",
  "found": true,
  "summary": "The UIDTLS_DtlsInd.aspx.cs file contains the code-behind for a web page designed for viewing details about individuals, including their personal information and relationships.",
  "purpose": "This file serves as the interface for viewing detailed information about an individual, including demographics, relationships, and other personal data.",
  "entities": [
    "UIDTLS_DtlsInd",
    "UIDTLS_DtlsNonClient"
  ],
  "fields": [
    "WPEventIn_Label",
    "WPLocIn_Label",
    "l_PID_5_PatientName",
    "l_PID_8_Sex",
    "PID_8_Sex",
    "l_PatientAKAs",
    "PatientAKAs",
    "l_PID_7_DateTimeOfBirth",
    "PID_7_DateTimeOfBirth",
    "l_Age",
    "Age",
    "l_PID_19_SsnNumberPatient",
    "PID_19_SsnNumberPatient",
    "l_PatientPHN",
    "PatientPHN",
    "l_Relationships",
    "Relationships",
    "b_Update",
    "b_New",
    "l_ClientDetails",
    "PID_5_PatientName_XPN_GivenName",
    "PID_5_PatientName_XPN_FamilylastName",
    "PID_5_PatientName_XPN_MiddleInitialOrName",
    "l_ID_HairColour",
    "l_Name",
    "b_Add",
    "b_Transfer",
    "l_NonClientDetails",
    "NID_2_NonClientContactPerson_XPN_GivenName",
    "NID_2_NonClientContactPerson_XPN_FamilylastName",
    "NID_2_NonClientContactPerson_XPN_MiddleInitialOrName",
    "NID_1_NonClientGeneral_SLDB_Title",
    "l_NID_1_NonClientGeneral_SLDB_Title",
    "l_NID_1_NonClientGeneral_SLDB_Organization",
    "NID_1_NonClientGeneral_SLDB_Organization",
    "l_NID_2_NonClientLocation_PhoneNumberHome_XTN_Label",
    "NID_2_NonClientLocation_PhoneNumberHome_XTN_AreaCode",
    "NID_2_NonClientLocation_PhoneNumberHome_XTN_PhoneNumber",
    "l_NID_2_NonClientLocation_PhoneNumberMobile_XTN_Label"
  ],
  "actors": [],
  "workflows": [
    {
      "name": "Page_Load",
      "steps": [
        "Register page with Anthem.Manager",
        "Initialize display if not postback"
      ]
    },
    {
      "name": "InitDisplay",
      "steps": [
        "Set PartyInCtxt value",
        "Get model data",
        "Initialize view with model data",
        "Fetch parentOrgId",
        "Set schedule number"
      ]
    }
  ],
  "business_rules": [
    "PartyInCtxt set to 'true' if CliInCtxt() is true"
  ],
  "validations": [],
  "calculations": [],
  "conditions": [
    "If !IsPostBack in Page_Load"
  ],
  "system_behavior": [
    "Register page",
    "Initialize view",
    "Set hidden inputs and labels",
    "Fetch model data",
    "Display client and non-client data"
  ],
  "dependencies": [
    "System.Web.UI.HtmlControls.HtmlInputHidden",
    "System.Web.UI.WebControls.Label",
    "System.Web.UI.WebControls.Button",
    "System.Web.UI.WebControls.DataGrid",
    "Sectorlynx.BaseViewControl",
    "Anthem.Manager"
  ],
  "exceptions": [],
  "content_gaps": []
}
```